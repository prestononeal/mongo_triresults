class Entrant
  ...
  # Flatten the RacerInfo, RaceRef, LegResult, and custom properties
  ...
  # use metaprogramming to set flattened event accessors
  ...
  RESULTS.keys.each do |name|
    ...
    define_method("#{name}=") do |event|
      case event
      when Event then attrs = event.attributes
      when Hash then attrs = event
      end
      event = self.send("#{name}").build_event(attrs)
    end
  ...
  def update_total(result)
    ...
    self.secs = 0
    results.each { |res| self.secs += res.secs if res.secs}
  end
